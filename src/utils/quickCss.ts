...
async function initSystemValues() {
    const values = await VelocityNative.themes.getSystemValues();
...
async function initThemes() {
    ...
    if (IS_WEB) {
        for (const theme of enabledThemes) {
            const themeData = await VelocityNative.themes.getThemeData(theme);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!IS_WEB) {
        VelocityNative.quickCss.addThemeChangeListener(initThemes);
    }
}, { once: true });
