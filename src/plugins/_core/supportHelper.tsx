...
async function generateDebugInfoMessage() {
    ...
    const commonIssues = {
        "NoRPC enabled": Velocity.Plugins.isPluginEnabled("NoRPC"),
        "Activity Sharing disabled": tryOrElse(() => !ShowCurrentGame.getSetting(), false),
        "Velocity DevBuild": !IS_STANDALONE,
...
function generatePluginList() {
    ...
    const enabledPlugins = Object.keys(plugins)
        .filter(p => Velocity.Plugins.isPluginEnabled(p) && !isApiPlugin(p));
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card className={`vc-warning-card ${Margins.top8}`}>
                ...
                <br />
                Instead, use the Velocity support channel: {Parser.parse("https://discord.com/channels/1015060230222131221/1026515880080842772")}
